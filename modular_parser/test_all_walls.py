...
# Outer walls
right_half = valid_cols[wall_x+300:w-10]
right_wall_x = wall_x + 300 + np.argmax(right_half) if len(right_half) > 0 else w

bottom_half = valid_rows[wall_y+300:h-10]
bottom_wall_y = wall_y + 300 + np.argmax(bottom_half) if len(bottom_half) > 0 else h

left_half = valid_cols[10:wall_x-300]
left_wall_x = 10 + np.argmax(left_half) if len(left_half) > 0 else 0

top_half = valid_rows[10:wall_y-300]
top_wall_y = 10 + np.argmax(top_half) if len(top_half) > 0 else 0
...
os.makedirs('isolated_models_v6', exist_ok=True)
cv2.imwrite('isolated_models_v6/model_top_left.png', model_1)
cv2.imwrite('isolated_models_v6/model_top_right.png', model_2)
cv2.imwrite('isolated_models_v6/model_bottom_left.png', model_3)
cv2.imwrite('isolated_models_v6/model_bottom_right.png', model_4)
